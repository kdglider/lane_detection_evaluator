# -*- coding: utf-8 -*-
"""
Created on Wed Mar 11 17:37:40 2020
...
class Algorithm2A:
    ...
    def camera_undistort(self, img, mtx, dist, debug=False):
        img_out = cv2.undistort(img, mtx, dist, None, mtx)
        return img_out

    def apply_CLAHE_LAB(self, img, debug=False):
    ...
    def do_perspective_transform(self, img, M, debug=False):
    ...
    def color_binary_thresh(self, img, debug=False):
        ...
        binary_color = np.zeros_like(binary_white)
        # binary_color[(binary_yellow == 1) | (binary_white == 1)] = 1
        binary_color[(binary_white == 1)] = 1
    ...
    def sobel_denoise(self, binary_img, kernel=5, thresh=0.7, debug=False):
    ...
    def sobel_x_thresh(self, img, sobel_kernel=3, thresh=(0, 255), debug=False):
    ...
    def sobel_mag_thresh(self, img, sobel_kernel=3, thresh=(0, 255), debug=False):
    ...
    def sobel_dir_thresh(self, img, sobel_kernel=3, thresh=(0, np.pi/2), debug=False):
    ...
    def sobel_magdir_combine(self, binary_mag, binary_dir, debug=False):
    ...
    def grad_combo_binary_thresh(self, img_gradx, img_magdir, debug=False):
    ...
    def combine_color_gradient_threshold(self, binary_color, binary_gradient, debug=False):
    ...
    def search_for_new_lanes(self, binary_warped, debug=False):
        N_WINDOWS = 8
        FOUND_MIN_PIX = 50 
        ...
        right_lane_xpts = []
        right_lane_ypts = []
        hist_peak_margin = np.int(HIST_PEAK_MARGIN_M / self.xm_per_pix)
        window_margin = np.int(NEW_WINDOW_MARGIN_M / self.xm_per_pix)
        lane_margin = np.int(NEW_LANE_MARGIN_M / self.xm_per_pix)
        ...
        self.left_lane.current_x = np.concatenate(left_lane_xpts)
        self.left_lane.current_y = np.concatenate(left_lane_ypts)
        self.right_lane.current_x = np.concatenate(right_lane_xpts)
        self.right_lane.current_y = np.concatenate(right_lane_ypts)
        
        if (len(self.left_lane.current_x) > 0) and (len(self.left_lane.current_x) == len(self.left_lane.current_y)):
            self.left_lane.current_fit = np.polyfit(self.left_lane.current_y, self.left_lane.current_x, 2)
        if (len(self.right_lane.current_x) > 0) and (len(self.right_lane.current_x) == len(self.right_lane.current_y)):
            self.right_lane.current_fit = np.polyfit(self.right_lane.current_y, self.right_lane.current_x, 2)
        
        
    def search_for_existing_lanes(self, binary_warped, debug=False):
        DETECTED_LANE_WINDOW_MARGIN = 0.3
        left_fit = self.left_lane.current_fit
        right_fit = self.right_lane.current_fit
        margin = np.int(DETECTED_LANE_WINDOW_MARGIN / self.xm_per_pix)
        ...
        self.left_lane.current_x = nonzerox[left_lane_inds]
        self.left_lane.current_y = nonzeroy[left_lane_inds] 
        self.right_lane.current_x = nonzerox[right_lane_inds]
        self.right_lane.current_y = nonzeroy[right_lane_inds]
    ...
    def lane_validity_check(self, binary_warped, debug=False):
        ...
        left_fit = self.left_lane.current_fit
        right_fit = self.right_lane.current_fit
        left_x_num = len(self.left_lane.current_x)
        left_y_num = len(self.left_lane.current_y)
        right_x_num = len(self.right_lane.current_x)
        right_y_num = len(self.right_lane.current_y)
        lane_width_min_pix = np.int(LANE_WIDTH_MIN_M / self.xm_per_pix)
        lane_width_max_pix = np.int(LANE_WIDTH_MAX_M / self.xm_per_pix)
        ...
        self.left_lane.detected = left_ok
        self.right_lane.detected = right_ok

    def update_lane_pts_and_polyfit(self):
        if self.left_lane.detected == True and self.right_lane.detected == True:
            while len(self.left_lane.all_x) >= self.left_lane.N_FITS:
                self.left_lane.all_x.pop(0)
                self.left_lane.all_y.pop(0)
            while len(self.right_lane.all_x) >= self.right_lane.N_FITS:
                self.right_lane.all_x.pop(0)
                self.right_lane.all_y.pop(0)

            self.left_lane.all_x.append(self.left_lane.current_x)
            self.left_lane.all_y.append(self.left_lane.current_y)
            self.right_lane.all_x.append(self.right_lane.current_x)
            self.right_lane.all_y.append(self.right_lane.current_y)

            left_lane_xpts = np.concatenate(self.left_lane.all_x)
            left_lane_ypts = np.concatenate(self.left_lane.all_y)
            right_lane_xpts = np.concatenate(self.right_lane.all_x)
            right_lane_ypts = np.concatenate(self.right_lane.all_y)

            self.left_lane.all_fit = np.polyfit(left_lane_ypts, left_lane_xpts, 2)
            self.right_lane.all_fit = np.polyfit(right_lane_ypts, right_lane_xpts, 2) 

    def make_lane_area_image(self, binary_warped, debug=False):
        left_fit = self.left_lane.all_fit
        right_fit = self.right_lane.all_fit
        left_x = self.left_lane.current_x
        left_y = self.left_lane.current_y
        right_x = self.right_lane.current_x
        right_y = self.right_lane.current_y
        ...
        if self.left_lane.detected == True and self.right_lane.detected == True:
            lane_area_color = (255, 255, 255)
        else:
            lane_area_color = (255, 255, 255)
        cv2.fillPoly(img_lane_area, np.int_([pts]), lane_area_color)
        img_lane_area[left_y, left_x] = [255, 255, 255]
        img_lane_area[right_y, right_x] = [255, 255, 255]
            
        return img_lane_area

    def calc_radius_and_offset(self, img, debug):
        leftx = np.concatenate(self.left_lane.all_x)
        lefty = np.concatenate(self.left_lane.all_y)
        rightx = np.concatenate(self.right_lane.all_x)
        righty = np.concatenate(self.right_lane.all_y)
        ...
        if (len(leftx) > 0) and (len(rightx) > 0):
            ...
            x_midpoint = np.int(img.shape[1]/2)*self.xm_per_pix

            self.left_lane.line_base_pos = x_midpoint - p_left(y_eval*self.ym_per_pix)
            self.right_lane.line_base_pos = p_right(y_eval*self.ym_per_pix) - x_midpoint



    def overlay_lane_area(self, img_undist, img_lane_lines, Minv, debug=False):
    # def overlay_text(image, text, pos):
            #cv2.putText(image, text, pos, fontFace=cv2.FONT_HERSHEY_SIMPLEX,fontScale=1, color=(0,0,0), thickness=10)
            #cv2.putText(image, text, pos, fontFace=cv2.FONT_HERSHEY_SIMPLEX,fontScale=1, color=(255,255,255), thickness=2)
            
        newwarp = cv2.warpPerspective(img_lane_lines, Minv, (img_lane_lines.shape[1], img_lane_lines.shape[0])) 
        blankcanv = np.zeros((img_undist.shape[0],img_undist.shape[1],3), dtype = 'uint8')
        #print(blankcanv.shape)
        overlaid_result = cv2.addWeighted(blankcanv, 1, newwarp, 1, 0)
        left_curverad = self.left_lane.radius_of_curvature
        right_curverad = self.right_lane.radius_of_curvature
        lane_offset = self.right_lane.line_base_pos - self.left_lane.line_base_pos
        if lane_offset < 0:
            side = 'right'
        else:
            side = 'left'
        #overlay_text(overlaid_result, 'Radius of curvature: L={:.0f} m, R={:.0f} m'.format(left_curverad, right_curverad), (50,100))
        #overlay_text(overlaid_result, 'Offset from lane center: {:.2f} m {}'.format(abs(lane_offset), side), (50,150))
        
        
        return overlaid_result

    def my_lane_pipeline(self, img, debug=False):
        K = [[9.037596e+02, 0.000000e+00, 6.957519e+02],
            [0.000000e+00, 9.019653e+02, 2.242509e+02], 
            [0.000000e+00, 0.000000e+00, 1.000000e+00]]
        dist = [[-3.639558e-01, 1.788651e-01, 6.029694e-04, -3.922424e-04, -5.382460e-02]]
        
        K = np.array(K)
        dist = np.array(dist)
        img_undist = self.camera_undistort(img, K, dist, debug=False)

        img_CLAHE = self.apply_CLAHE_LAB(img_undist, debug=False)

        H, H_inv = self.set_perspective_transform(img_undist)
        img_warped = self.do_perspective_transform(img_undist, H, debug=False)
        
        binary_color = self.color_binary_thresh(img_CLAHE, debug=False)
        binary_color_warped = self.do_perspective_transform(binary_color, H, debug=False)

        if (self.left_lane.detected == False) or (self.right_lane.detected == False):
            binary_gradx_warped = self.sobel_x_thresh(img_warped, thresh=(10, 255), debug=False)
            binary_gradx_warped = self.sobel_denoise(binary_gradx_warped, kernel=5, thresh=0.7, debug=False)
            binary_mag = self.sobel_mag_thresh(img_undist, sobel_kernel=3, thresh=(5, 255), debug=False)
            binary_dir = self.sobel_dir_thresh(img_undist, sobel_kernel=3, thresh=(0.5, 1.3), debug=False)
            binary_magdir = self.sobel_magdir_combine(binary_mag, binary_dir, debug=False)
            binary_magdir = self.sobel_denoise(binary_magdir, kernel=5, thresh=0.7, debug=False)
            binary_magdir_warped = self.do_perspective_transform(binary_magdir, H, debug=False)
            binary_gradient_warped = self.grad_combo_binary_thresh(binary_gradx_warped, binary_magdir_warped, debug=False)
        else:
            binary_gradient_warped = None

        binary_final = self.combine_color_gradient_threshold(binary_color_warped, binary_gradient_warped, debug=False)
        
        if (self.left_lane.detected == False) or (self.right_lane.detected == False):
            self.search_for_new_lanes(binary_final, debug=False)
        else:
            self.search_for_existing_lanes(binary_final, debug=False)
            
        self.lane_validity_check(binary_final, debug=False)
        self.update_lane_pts_and_polyfit()
        img_lane_lines = self.make_lane_area_image(binary_final, debug=False)

        self.calc_radius_and_offset(img_lane_lines, debug=False)
        
        overlaid_result = self.overlay_lane_area(img_undist, img_lane_lines, H_inv, debug)
    ...
    def detectLane(self, frame):
        ...
        lanes = self.my_lane_pipeline(frame)
...
if __name__ == '__main__':
    ...
    #out = cv2.VideoWriter('data1_output.mp4', fourcc, 30, (1392, 512))
    #while(cap.isOpened()):
        #ret, frame = cap.read()
        #f ret == False:
         #   break
