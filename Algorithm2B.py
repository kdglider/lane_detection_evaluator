...
# -*- coding: utf-8 -*-
"""
Created on Mon May 11 03:40:59 2020
...
class Algorithm2B:
    ...
    def get_curve(self, img, leftx, rightx):
        ploty = np.linspace(0, img.shape[0]-1, img.shape[0])
        y_eval = np.max(ploty)
        ym_per_pix = 30.5/375 # meters per pixel in y dimension
        xm_per_pix = 3.7/375 # meters per pixel in x dimension
    ...
    def draw_lanes(self, img, left_fit, right_fit):
        ploty = np.linspace(0, img.shape[0]-1, left_fit.shape[0])
        ...
        cv2.fillPoly(color_img, np.int_(points), (255,255,255))
        inv_perspective = self.inv_perspective_warp(color_img)
        blankcanvas = np.zeros_like(inv_perspective)
        inv_perspective = cv2.addWeighted(blankcanvas, 1, inv_perspective, 1, 0)
        return inv_perspective

    def vid_pipeline(self, img):
    ...
    def detectLane(self, frame):
        img1 = self.pipeline(frame)
        img2 = self.perspective_warp(img1)
        out_img, curves, lanes, ploty = self.sliding_window(img2, draw_windows=False)
        curverad = self.get_curve(img2, curves[0], curves[1])
        lane_curve = np.mean([curverad[0], curverad[1]])
        img3 = self.draw_lanes(frame, curves[0], curves[1])
...
if __name__ == '__main__':
    ...
    destFolder = '/Users/sarjana/Documents/UMD /Spring 2020/ENSE 622/dataset/Hackster_Code'
    filename = os.listdir(sourceFolder)
    for file in filename :
        if file.endswith('.png') :
            img = cv2.imread(sourceFolder+ '/'+ file)
            img1 = pipeline(img)
            img2 = perspective_warp(img1)
            out_img, curves, lanes, ploty = sliding_window(img2, draw_windows=False)
            curverad =get_curve(img2, curves[0], curves[1])
            lane_curve = np.mean([curverad[0], curverad[1]])
            img3 = draw_lanes(img, curves[0], curves[1])
            cv2.imwrite(destFolder + '/' + file,img3)
    '''